...
describe("findSilences", () => {
  ...
  it("handles audio that is silent throughout", () => {
    expect(findSilences([QUIET, QUIET, QUIET, QUIET], 0.1)).toEqual([
      { start: 0, end: 0.4 },
    ]);
  });
  ...
  it("honours a custom threshold", () => {
    ...
    expect(
      findSilences([LOUD, QUIET, QUIET, QUIET, QUIET, LOUD], 0.1, {
        thresholdDb: -70,
      }),
    ).toEqual([]);
  });
...
describe("toKeepSegments", () => {
  it("inverts silences into speech and pads both edges", () => {
    const keep = toKeepSegments([{ start: 2, end: 3 }], 5, {
      paddingSeconds: 0.04,
    });
    expect(keep).toEqual([
      { start: 0, end: 2.04 },
  ...
  it("never pads past the start or end of the file", () => {
    const keep = toKeepSegments([{ start: 1, end: 2 }], 3, {
      paddingSeconds: 0.5,
    });
  ...
  it("merges segments that padding pushed into each other", () => {
    ...
    const keep = toKeepSegments([{ start: 1, end: 1.05 }], 3, {
      paddingSeconds: 0.04,
    });
...
describe("adjustEdgeTrim", () => {
  // The preview screen's fine-edit control: small nudges to the very first
  // and very last cut point, on top of whatever silence detection produced.
  const segments = [
    { start: 1, end: 2 },
    { start: 3, end: 8 },
  ];

  it("does nothing to an empty segment list", () => {
    expect(adjustEdgeTrim([], "start", 0.2, 10)).toEqual([]);
  });

  it("moves the first segment's start later, leaving the rest untouched", () => {
    const result = adjustEdgeTrim(segments, "start", 0.5, 10);
    expect(result[0]).toEqual({ start: 1.5, end: 2 });
    expect(result[1]).toEqual({ start: 3, end: 8 });
  });

  it("moves the first segment's start earlier, never below zero", () => {
    const result = adjustEdgeTrim(segments, "start", -5, 10);
    expect(result[0].start).toBe(0);
  });

  it("never trims a segment shorter than the minimum", () => {
    // The segment is only 1s long; trimming 0.95s past its own end minus the
    // minimum would invert start and end.
    const result = adjustEdgeTrim(segments, "start", 0.95, 10, {
      minSegmentSeconds: 0.15,
    });
    expect(result[0].end - result[0].start).toBeCloseTo(0.15, 6);
  });

  it("moves the last segment's end earlier, leaving the rest untouched", () => {
    const result = adjustEdgeTrim(segments, "end", -1, 10);
    expect(result[1]).toEqual({ start: 3, end: 7 });
    expect(result[0]).toEqual({ start: 1, end: 2 });
  });

  it("moves the last segment's end later, never past the clip duration", () => {
    const result = adjustEdgeTrim(segments, "end", 5, 10);
    expect(result[result.length - 1].end).toBe(10);
  });

  it("leaves the input array untouched", () => {
    const before = JSON.stringify(segments);
    adjustEdgeTrim(segments, "start", 0.5, 10);
