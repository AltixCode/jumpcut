...
export default function HomeScreen() {
  ...
  const handleCut = useCallback(async () => {
    ...
    // Checked before spending time on the cut, not after: a permission
    // denial found only once the encode is done is a wasted wait for a
    // result the user still cannot save.
    ...
    try {
      ...
      // The library write and the "saved" confirmation now live on the
      // preview screen -- this only stages the result for the user to watch
      // and confirm, per the TestFlight request for a preview before saving.
      setResult(result.uri, result.duration, segments);
      router.push("/preview");
    } catch (error) {
      // Only on failure: setResult already moved the stage to "previewing"
      // on success, and resetting it here would undo that the instant the
  ...
  }, [exportableKeep, router, setProgress, setResult, setStage, source]);
      ...
      >
        ...
        {!analysed ? (
          <TouchableOpacity
            ...
            style={{
              backgroundColor:
                source && !busy ? theme.primary : theme.controlSurface,
              minHeight: 44,
            }}
          >
            {busy ? (
              ...
              <Wind
                size={18}
                color={source ? theme.onPrimary : theme.textMuted}
              />
            )}
            <Text
              className="font-bold text-base ml-2 mr-2"
              style={{
                color: source || busy ? theme.onPrimary : theme.textMuted,
              }}
            >
        ...
        {analysed ? (
          ...
          >
            ...
            {[
              {
                label: t("silenceCount", { count: silences.length }),
                value: "",
              },
            ...
            <Text
              className="text-xs font-semibold tracking-widest mt-3 mb-2"
            ...
            >
              ...
              >
                {stage === "cutting"
                  ? t("cutting", { percent: Math.round(progress * 100) })
                  : t("cut")}
              </Text>
            </TouchableOpacity>
          </View>
        ...
        {[
          {
            icon: <Wind size={18} color={theme.primary} />,
            bg: theme.primaryLight,
            title: t("paddingTitle"),
            desc: t("paddingDesc"),
          },
          {
            icon: <Timer size={18} color={theme.accent} />,
            bg: theme.accentLight,
            title: t("preciseTitle"),
            desc: t("preciseDesc"),
          },
          {
            icon: <ShieldCheck size={18} color={theme.success} />,
            bg: theme.successLight,
            title: t("onDeviceTitle"),
            desc: t("onDeviceDesc"),
          },
        ].map((item) => (
          <View
